...
def get_file_paths(loc):
    ##DataLoc
    dat0 = '/root/data/'
    dat1 = '/*.tfrecords'
    filePaths = glob.glob(dat0+loc+dat1)
    print('Length of ', loc, 'is ', len(filePaths))
    return filePaths
...
if __name__=="__main__":
    ...
    full_train = get_file_paths('train')
    full_test1 = get_file_paths('test-1')
    full_test2 = get_file_paths('test-2')
    full_valid = get_file_paths('validate')
    ...
    estimator.train(input_fn=lambda:tfrecord_input_fn(full_train, hparams, shuffle_buf=10000), 
                max_steps=5)
    # estimator.evaluate(input_fn=lambda:tfrecord_input_fn(full_test1, hparams, shuffle_buf=10000, target='delay'), 
    #              steps=3398)